...
interface EvaluationWizardStepsProps {
  currentStep: number;
...
export function EvaluationWizardSteps({
  ...
  const progress = ((currentStep - 1) / (EVALUATION_WIZARD_STEPS.length - 1)) * 100;
    ...
    <div className="w-full">
      ...
      <div className="relative mb-8">
        ...
        <div className="relative flex justify-between">
          {EVALUATION_WIZARD_STEPS.map((step) => {
      ...
      <div className="sm:hidden text-center mb-4">
        <p className="text-sm text-muted-foreground">
          Passo {currentStep} de {EVALUATION_WIZARD_STEPS.length}
        </p>
        <p className="font-medium">{EVALUATION_WIZARD_STEPS[currentStep - 1]?.title}</p>
        <p className="text-sm text-muted-foreground">
