...
-- 1. Adicionar 'pending' ao enum employee_status
ALTER TYPE employee_status ADD VALUE IF NOT EXISTS 'pending' BEFORE 'active';

-- 2. Atualizar handle_new_user para lidar com employee existente com status pending
...
BEGIN
  ...
  IF pending IS NOT NULL THEN
    ...
    IF existing_employee_id IS NOT NULL THEN
      -- Atualizar employee existente: trocar ID para o auth.uid e ativar
